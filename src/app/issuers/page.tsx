 ...
 const addTrustedIssuer = async()=>
  {
    const selectedWalletSWO = await connect({ modalMode: 'neverAsk', modalTheme: 'light' });
    const myWalletAccount = new WalletAccount({}, selectedWalletSWO);
    const contract = new Contract(identityABI, did, myWalletAccount);
   setIsSaving(true) 
   const address = document.getElementById("address").value
    console.log(typeof BigInt(address))
    console.log(address.length) 
   if(address.length < 65 ||  typeof BigInt(address) != 'bigint')
   {
    setDialogType(2) //error
    setNotificationTitle("Add Trusted Issuer")
    setNotificationDescription("Invalid trusted issuer address .")
    setShow(true)
   ...
   try{
         ...
         const   tx = await contract.add_issuer(address)
          const txR = await myWalletAccount.waitForTransaction(tx.transaction_hash); 
          setDialogType(1) //Success
          setNotificationTitle("Add Trusted Issuer")
          setNotificationDescription("Trusted Issuer added.")
          setShow(true)
          setIsSaving(false)
   ...
   }catch(error)
   {
        console.log(error)
          ...
          setDialogType(2) //error
          setNotificationTitle("Add Trusted Issuer")
          setNotificationDescription("Trusted Issuer not added.")
          setShow(true)
